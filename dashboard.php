...
<div class="sidenav">
  ...
  <a href="contactus.php">Contact</a>
		...
		<div class="container" style="margin-left: 155px;width:">
			    <h4 style="margin-left: 45px;">Order details</h4>
				<div class="col-lg-12">
	...
	$q = " SELECT `id`,`username`, `quantity`,`payment`,`watertype`,`transaction` FROM `user` order by id ASC ";
	...
	if($n>0){
		while($user=mysqli_fetch_array($que)){
           $set=$user['username'];
	    $query = " SELECT * FROM `reg` where username='$set'";
        $queryfire = mysqli_query($con, $query);
			...
			?>
					<tr>
						...
						<td> <button class="btn" style="background-color: #1159B7;"> <a href="qdelete.php?id=<?php echo $user['id']; ?>" class="text-white">Deliverd</a>  </button> </td>
					</tr>
					<?php
					   break;}
					}
